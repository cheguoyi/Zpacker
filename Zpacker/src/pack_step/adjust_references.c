...
bool adjust_references(size_t shift_amount, struct entry *original_entry){
    ...
    adjust_phdr_table_offset(elf_header);
...
void adjust_sheader_table_offset(Elf64_Ehdr *header){
    Elf64_Off sec_header_off=endian_8(header->e_shoff);
    if(sec_header_off<closure.end_last_sect){
        return;
    }
...
void	adjust_phdr_table_offset(Elf64_Ehdr *safe_elf_hdr)
{
	...
	if (e_phoff < closure.end_last_sect)
		return ;

	e_phoff              += closure.shift_amount;
...
bool shift_seg_header_position(check_safe_func_pointer csf,const size_t offset){
    ...
    if(seg_offset>closure.end_last_sect){
        ...
        seg_header->p_offset=endian_8(seg_offset);
    }
    return true;
}
